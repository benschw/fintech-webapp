...
class Routing_RouteMap {
	...
	public function connect( $key, Routing_Route $route ) {
		if( isset( $this->indexed[$key] ) ) {
			throw new Routing_RouteException( "Route '{$key}' already exists" );
		}
	...
	public function urlFor( $key, array $params = array() ) {
		if( !isset( $this->indexed[$key] ) ) {
			throw new Routing_RouteException( "'" . $key . "' not found in routes" );
		}
	...
	public function getRouteByKey( $key ) {
		if( !isset( $this->indexed[$key] ) ) {
			throw new Routing_RouteException( "Route '{$key}' does not exist" );
		}
		return $this->indexed[$key];
	}
	public function getRouteKey( Web_Uri $request ) {
		foreach( $this->indexed AS $key => $route ) {
			if( $route->match( $request ) ) {
				return $key;
			}
		}
		throw new Routing_RouteException( "No Match" );
	}
	...
	public function getRoute( Web_Uri $request ) {
		if ( substr( (string)$request, 0, 1 ) === '@' ) {
			...
			if ( array_key_exists( $key, $this->indexed ) ) {
				...
				throw new Routing_RouteException( sprintf( "Internal Route Not Found for '%s'", $request ) );
			}
		}
		...
		throw new Routing_RouteException( "Route Not Found" );
	}
