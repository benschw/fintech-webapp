...
class Routing_PatternRoute extends Routing_RegexRoute {
	protected $pattern;
	...
	static public function parse($pattern) {
		return array(
			'/^' . preg_replace_callback('/\\\:(\w+)/', 'Routing_PatternRoute::__parser_callback', preg_quote($pattern, '/')) . '$/',
	...
	public function urlFor(array $params = array()) {
		...
		foreach ($this->toCapture as $key) {
			if (!array_key_exists($key, $params)) {
				throw new Routing_RouteException('Missing parameter "' . $key . '" in params');
			}
		...
		return Web_Uri::get('/' . $this->namespace . '/')->join(new Web_Uri($this->assembleUrl($url, $params)));
	}
