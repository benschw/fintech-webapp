...
class Flfc_FcChainFactory {
	...
	public static function setChain(Flfc_App $chain) {
		self::$oldChain = $chain;
	}

	public static function addResolver(Flfc_ResolvableFcChain $chain) {
	 ...
	 */
	public static function getChainRunner(Flfc_Context $context) {
		$chain = null;
		...
		return new Flfc_FcChainRunner($chain);
	}

	public static function getChain(Flfc_Context $context) {
		$chain = null;
		foreach (self::$resolvers as $resolver) {
			if ($resolver->canResolve($context)) {
				$chain = $resolver->getChain();
